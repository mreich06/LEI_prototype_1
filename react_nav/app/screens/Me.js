...
import { VictoryBar, VictoryChart, VictoryAxis, VictoryStack, VictoryTheme, VictoryGroup, VictoryLabel, VictoryLine } from "victory-native";
...
class Me extends Component {
  ...
  render() {
    ...
    if (this.state.data == "GRILL") {
      ...
      percentData = grill.family.list.map((avg) => avg.percent);
      if (this.state.data2 == "MONDAY") {
    ...
    if (prevData) {
      ...
      compiledData2.splice(0, compiledData2.length);
      weightAvg.splice(0, weightAvg.length);
      weightAvg2.splice(0, weightAvg2.length);
      for (var i = 0; i < cycleData.length; i++) {
        ...
        compiledData2.push([{product: 1, amount: cycleData[i]}]);
        weightAvg.push([{product: cycleData.length+1, amount: cycleData[i]*(percentData[i]/100)}]);
        weightAvg2.push([{product: 1, amount: cycleData[i]*(percentData[i]/100)}]);

      }

      for (var i=0; i<weightAvg.length; i++) {
        avgLabel += weightAvg[i];
      }
    ...
    for (var k = 0; k < cycleData.length; k++) {
      victoryBarList2.push(<VictoryBar
        data={weightAvg2[k]}
        x="product"
        y="amount"
        key={k}
    ...
    return (
      ...
      <ScrollView>
        <View style={styles.allContents}>
          <View style={styles.content}>
            <View style={styles.titleBox}>
              <Text style={styles.titleBoxText}>Station</Text>
            </View>
        ...
        >
          ...
          <View style={styles.content}>
            <View style={styles.titleBox}>
              <Text style={styles.titleBoxText}>Day</Text>
            </View>
            ...
            <Text>{this.state.data} Station Rate of Order, </Text>
            <Text>{this.state.data2} Peak Period</Text>
            </View>
